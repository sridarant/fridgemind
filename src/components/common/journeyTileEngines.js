...
// getPersonalisedPicks:  Zone 3 ranked tiles (shortcut buttons)
// getScoredForYouCards:  Zone 5 scored "For You" cards from recommendationService
...
function getFeaturedTile({ festival, sports, weather, dayCtx, profile, isReturning, lastFavCuisine }) {
  ...
  if (festival && sports) {
    if (new Date().getHours() >= 14) return {
      ...
      label: sports.label, sub: sports.note,
...
function getPersonalisedPicks({ profile, ratings }) {
  ...
  const all = [
    ...
    { id:'goal',    emoji:'🎯',
      label: profile && profile.active_goal === 'eat_healthier' ? 'Eating healthier'
           : profile && profile.active_goal === 'cook_faster'   ? 'Quick meals'
           : profile && profile.active_goal === 'reduce_waste'  ? 'Zero waste'
           : profile && profile.active_goal === 'try_new_things'? 'Try something new'
           : 'My goal',
      sub: (profile && profile.active_goal) ? 'On track with your goal' : 'Plans & targets',
...
// ── Scored "For You" cards — Zone 5 ─────────────────────────────────
// Returns up to 3 card objects. why is now a rich object: { headline, bullet2, effortLabel, effortMins }
function getScoredForYouCards({ profile, ratings, mealHistory, overrideMealType }) {
  ...
  return recs.map(rec => ({
    ...
    tags:    rec.meal.tags,
    why:     rec.why,        // { headline, bullet2, effortLabel, effortMins }
    role:    rec.role,
    score:   rec.score,
