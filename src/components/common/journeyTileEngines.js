...
// getFeaturedTile:     Zone 6 context card (festival/sports/weather/day)
// getScoredForYouCards: thin wrapper kept for any external callers
...
// Thin wrapper (kept for backward compat — JourneyTiles now calls engine directly)
function getScoredForYouCards({ profile, ratings, mealHistory, overrideMealType }) {
  const recs = getPersonalisedRecommendations({
    profile: profile || null, ratings: ratings || {},
    mealHistory: mealHistory || [], overrideMealType: overrideMealType || null,
  });
  return recs.map(rec => ({
