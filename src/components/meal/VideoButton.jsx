...
// Shared inline video button for all recipe cards across the app.
// Shows a tap-to-fetch button. On tap: calls /api/videos, shows inline iframe.
...
export function VideoButton({ recipeName, cuisine = '', lang = 'en', compact = false }) {
  ...
  if (state === 'found' && videoData) return (
    <div style={{borderRadius:10, overflow:'hidden', border:'1px solid rgba(204,0,0,0.2)', marginTop:8}}>
      ...
      />
      <div style={{
        padding:'5px 10px', background:'rgba(204,0,0,0.04)',
        ...
        <button onClick={()=>setState('idle')}
          style={{background:'none', border:'none', cursor:'pointer', fontSize:12, color:'#7C6A5E', flexShrink:0, marginLeft:4}}>
          ✕
        </button>
      </div>
    </div>
  ...
  // notfound — YouTube search fallback
  return (
    <div style={{display:'flex', alignItems:'center', gap:8, padding:'6px 0'}}>
      <a href={ytFallback} target="_blank" rel="noopener noreferrer"
        style={{
      ...
      <button onClick={()=>setState('idle')}
        style={{background:'none', border:'none', cursor:'pointer', fontSize:12, color:'#7C6A5E'}}>
        ✕
      </button>
    </div>
  );
