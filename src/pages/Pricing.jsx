...
export default function Pricing() {
  ...
  const handleUpgrade = async (planId) => {
    if (!razorpayEnabled) { activateTestPremium(); setSuccess(true); return; }
  ...
  const hdr = {
    ...
    padding: '18px 36px', borderBottom: `1px solid ${C.border}`,
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '56px 24px 80px' }}>
        ...
        {isPremium && (
          <div style={{ textAlign: 'center' }}>
            ...
            <h1 style={{ fontFamily: "'Fraunces', serif", fontSize: 'clamp(32px,5vw,48px)', fontWeight: 900, color: C.ink, letterSpacing: '-1.5px', marginBottom: 12 }}>Welcome to the good stuff.</h1>
            <p style={{ fontSize: 16, color: C.muted, fontWeight: 300, lineHeight: 1.7, marginBottom: 24 }}>
              Plan: <strong>{premium?.planId}</strong> · Expires: {premium?.expiresAt ? new Date(premium.expiresAt).toLocaleDateString('en-IN') : 'Never'}
            </p>
        ...
        {!isPremium && success && (
          <div style={{ textAlign: 'center' }}>
            ...
            <h1 style={{ fontFamily: "'Fraunces', serif", fontSize: 'clamp(32px,5vw,48px)', fontWeight: 900, color: C.ink, letterSpacing: '-1.5px', marginBottom: 12 }}>Welcome to Jiff Premium!</h1>
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px,1fr))', gap: 14, marginBottom: 28 }}>
              {planList.map(plan => {
                ...
                return (
                  <div key={plan.id} onClick={() => setSelected(plan.id)} style={{ background: sel ? C.jiff : 'white', border: `2px solid ${sel ? C.jiff : C.borderMid}`, borderRadius: 18, padding: '22px 20px', cursor: 'pointer', transition: 'all 0.18s', position: 'relative', boxShadow: sel ? '0 8px 28px rgba(255,69,0,0.25)' : C.shadow }}>
                    ...
                    <div style={{ fontFamily: "'Fraunces', serif", fontSize: 30, fontWeight: 900, color: sel ? 'white' : C.ink, letterSpacing: '-1px', marginBottom: 4 }}>{plan.price}</div>
            ...
            <div style={{ textAlign: 'center', marginBottom: 10 }}>
              <button
                ...
                {loading ? '⏳ Processing…' : `⚡ Upgrade to ${selectedPlan?.label || 'Premium'}`}
              </button>
            </div>

            {!razorpayEnabled && (
              <div style={{ textAlign: 'center', fontSize: 12, color: C.muted, marginBottom: 12 }}>
                ℹ️ Razorpay not configured — clicking upgrade activates free test premium.
              </div>
            )}
            ...
            <div style={{ textAlign: 'center', marginTop: 48 }}>
              ...
              <button onClick={() => handleUpgrade(selected)} disabled={!!loading} style={{ background: C.jiff, color: 'white', border: 'none', borderRadius: 14, padding: '16px 44px', fontSize: 16, fontWeight: 500, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}>
                ⚡ Get Premium →
              </button>
            </div>
          </>
